...
if(isset($data['user_id']) && isset($data['prop_id']) && isset($data['ballot'])){
	...
	if($user_code == $ballot_decoded['user_code']){
		if($ballot_decoded['user_id'] == $data['user_id']){
			$post_vars = Array("user_code" => $user_code, "prop_id" => $data['prop_id'], "rsa" => $ballot_decoded['rsa'], "aes" => $ballot_decoded['aes']);
	...
	else echo "failed - user code error: stored in ballot was " . $ballot_decoded['user_code'] . " but expected " . $user_code;
}
